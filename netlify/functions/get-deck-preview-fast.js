...
// Best-effort zone extraction. Curiosa may return one mixed card array tagged
// with a board/category field, OR separate arrays -- handle both, with no
// change to the main list when neither is present.
function entryBoard(e) {
  var b = (e && (e.board || e.category || e.zone || e.section || e.list || e.type)) || '';
  return String(b).toLowerCase();
}
function splitByBoard(list) {
  var main = [], coll = [], maybe = [];
  (list || []).forEach(function (e) {
    var b = entryBoard(e);
    if (b.indexOf('maybe') >= 0 || b.indexOf('sideboard') >= 0) maybe.push(e);
    else if (b.indexOf('collection') >= 0) coll.push(e);
    else main.push(e);
  });
  return { main: main, coll: coll, maybe: maybe };
}
function findListByKeys(obj, keys, depth) {
  if (!obj || typeof obj !== 'object' || depth > 6) return null;
  for (const k of Object.keys(obj)) {
    if (keys.includes(k.toLowerCase()) && Array.isArray(obj[k]) && obj[k].length) return obj[k];
  }
  for (const k of Object.keys(obj)) {
    const f = findListByKeys(obj[k], keys, depth + 1);
    if (f) return f;
  }
  return null;
}
...
exports.handler = async function (event) {
  ...
  try {
    ...
    const _split = splitByBoard(cardList);
    const _maybe = _split.maybe.length ? _split.maybe : (findListByKeys(deckData, ['maybeboard', 'maybe', 'sideboard'], 0) || []);
    const _coll = _split.coll.length ? _split.coll : (findListByKeys(deckData, ['collection'], 0) || []);
    return { statusCode: 200, headers: jsonHeaders, body: JSON.stringify({ ok: true, deckName: deckData.name, cards: _split.main, collection: _coll, maybeboard: _maybe }) };
  } catch (err) {
